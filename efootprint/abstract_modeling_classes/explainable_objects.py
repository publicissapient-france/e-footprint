...
class ExplainableHourlyQuantities(ExplainableObject):
    ...
    def create_empty_hourly_dimensionless_quantities_with_same_index(self):
        copy_df = self.value.copy()
        copy_df["value"] = pint_pandas.PintArray([0] * len(self.value), dtype=u.dimensionless)
    ...
    def __add__(self, other):
        if issubclass(type(other), numbers.Number) and other == 0:
            ...
            tmp_sum = self.value.add(other.value, fill_value=0 * self.unit)
            aligned_index_sum = tmp_sum.combine_first(self.value).combine_first(other.value)
            return ExplainableHourlyQuantities(aligned_index_sum, "", self, other, "+")
        else:
