...
class TestAutoscaling(TestCase):
    ...
    def test_nb_of_instances_autoscaling_simple_case(self):
        ram_need = SourceHourlyValues(create_hourly_usage_df_from_list([0, 1, 2, 5], pint_unit=u.GB))
        cpu_need = SourceHourlyValues(create_hourly_usage_df_from_list([1, 4, 2, 10], pint_unit=u.core))
        ...
        with patch.object(self.server_base, "all_services_ram_needs", new=ram_need), \
            ...
            self.assertEqual([1, 1, 1, 3], self.server_base.nb_of_instances.value_as_float_list)
